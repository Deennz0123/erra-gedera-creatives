...
def analyze(path, size):
    ...
    if cc is None:
        ...
        if cc["share"] < 0.30:
            print("  → РЕЖИМ II: начисление приходит разрывом, внутри сессии цена почти стоит.")
            print("    Простой внутри дня не стоит почти ничего, отбор исполнений против нас")
            print("    исчезает, пассивный цикл даёт полный тик. Лучший из двух режимов.")
        else:
            print("  → РЕЖИМ I: дрейф идёт внутри сессии, работает модель фаз.")
            print("    Исполнения отобраны против нас, реальный захват — доля тика.")
